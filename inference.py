...
class Inference(object):
    ...
    def predict_cls(self, sentence, prompt=None):
        if self.service.__contains__('hug'):
            ...
            text_class=completion.choices[0].text.strip().lower()
            ...
            # print(text_class)
            # print(np.exp(a[0]))
            if "positive" in text_class:
                pred["pred_conf"]=[1-np.exp(a[0]),np.exp(a[0])]
                pred["pred_class"] = 1
            elif "positi" in text_class:
                pred["pred_conf"]=[1-np.exp(a[0]),np.exp(a[0])]
                pred["pred_class"] = 1
            elif "posi" in text_class:
                pred["pred_conf"]=[1-np.exp(a[0]),np.exp(a[0])]
                pred["pred_class"] = 1
            elif "pos" in text_class:
                pred["pred_conf"]=[1-np.exp(a[0]),np.exp(a[0])]
                pred["pred_class"] = 1
            elif " positive" in text_class:
                pred["pred_conf"]=[1-np.exp(a[0]),np.exp(a[0])]
                pred["pred_class"] = 1
            elif "positive" in text_class:
                pred["pred_conf"]=[1-np.exp(a[0]),np.exp(a[0])]
            ...
            elif "negative" in text_class:
                pred["pred_conf"]=[np.exp(a[0]),1-np.exp(a[0])]
                pred["pred_class"] = 0
            elif "ne" in text_class:
                pred["pred_conf"]=[np.exp(a[0]),1-np.exp(a[0])]
                pred["pred_class"] = 0
            elif "neg" in text_class:
                pred["pred_conf"]=[np.exp(a[0]),1-np.exp(a[0])]
                pred["pred_class"] = 0
            elif "nega" in text_class:
                pred["pred_conf"]=[np.exp(a[0]),1-np.exp(a[0])]
                pred["pred_class"] = 0
            elif "negati" in text_class:
                pred["pred_conf"]=[np.exp(a[0]),1-np.exp(a[0])]
                pred["pred_class"] = 0
            else:
                pred["pred_conf"]=[np.exp(a[0]),1-np.exp(a[0])]
            ...
            if self.model == 'text-davinci-002':
                ...
                print(Prompt_input)
            conversation_list = []
            conversation_list.append({"role":"user","content":Prompt_input})
            completion = openai.ChatCompletion.create(
                model="gpt-3.5-turbo",
                messages=conversation_list 
            )
            pred = {}
            text_class = completion.choices[0].message['content'].strip().lower()
